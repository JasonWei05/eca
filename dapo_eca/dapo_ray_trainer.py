...
class RayDAPOTrainer(RayPPOTrainer):
    ...
    def compute_kl_related_metrics(self, batch: DataProto, metrics: dict, timing_raw: dict):
        ...
        log.info("[DAPO] compute_kl_related_metrics: START")
        ...
        with marked_timer("old_log_prob", timing_raw, "blue"):
            ...
            if "sum_pi_squared" in old_log_prob.batch.keys():
                ...
                if self.config.algorithm.get("linear", False):
        ...
        use_vn_entropy = self.config.trainer.get("use_vn_entropy", False)
        if use_vn_entropy:
            pca_freq = self.config.trainer["pca_freq"]
            pca_dim = self.config.trainer["pca_dim"]
            top_k = self.config.trainer["vn_entropy_top_k"]
            chunk_size = self.config.trainer["vn_entropy_chunk_size"]
            if pca_freq and pca_freq > 0:
                last_pca_step = getattr(self, "_vn_entropy_last_pca_step", None)
                if last_pca_step != self.global_steps and self.global_steps % pca_freq == 0:
                    log.info(f"[DAPO] update_vn_entropy_pca: START (pca_dim={pca_dim}, top_k={top_k})")
                    with marked_timer("update_pca", timing_raw, "purple"):
                        self.actor_rollout_wg.update_vn_entropy_pca(pca_dim=pca_dim, top_k=top_k)
                    self._vn_entropy_last_pca_step = self.global_steps
                    log.info("[DAPO] update_vn_entropy_pca: DONE")

            batch.meta_info["vn_entropy_chunk_size"] = chunk_size
            batch.meta_info["vn_entropy_top_k"] = top_k
            batch.meta_info["vn_entropy_pca_dim"] = pca_dim
            log.info(f"[DAPO] compute_vn_entropy: START (chunk_size={chunk_size}, top_k={top_k}, pca_dim={pca_dim})")
            with marked_timer("vn_entropy", timing_raw, "purple"):
                vn_entropy_out = self.actor_rollout_wg.compute_vn_entropy(batch)
                log.info("[DAPO] compute_vn_entropy: worker returned")
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    # Linear advantage reweighting: redistribute per-token by gradient magnitude
                    if self.config.algorithm.get("linear", False) and "grad_sq" in batch.batch:
                        response_mask = batch.batch["response_mask"]
                        grad_sq = batch.batch["grad_sq"] * response_mask
                        T = response_mask.sum(dim=-1, keepdim=True)
                        sum_G = grad_sq.sum(dim=-1, keepdim=True).clamp(min=1e-10)
